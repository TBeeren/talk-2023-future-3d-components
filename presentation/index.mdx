...
And yeah, they might have put all their time and energy into creating the 3D experience and not that much into the buggy text 😆
-->
...
import CpuGpu from '../assets/cpu_gpu.png'

CPU vs GPU
<img src={CpuGpu} style={{ width: '80%', marginBottom: '20px'}} />

<!--
When talking about 3D, there is no way to escape some slight low-level knowledge of the hardware.

While a typical CPU today will have anywhere between 2 and 16 cores, a GPU typically have thousands of cores.
Rather than processing tasks serially (in order) like a CPU, a GPU breaks up tasks and uses parallel processing for rendering.
These processes are insanely optimised so that the GPU is able to render computational heavy renders within a blink of your eye.
-->
